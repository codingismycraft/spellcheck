...
static void display_grid(int rows, int cols){
...
int wdist(const char* psz1, const char* psz2, bool verbose){
    ...
    const int l1 = strlen(psz1);
    const int l2 = strlen(psz2);
    ...
    for (int row= 1; row<= l1; ++row){
        ...
        for (int col= 1; col<= l2; ++col){
            ...
            const int upper = buffer[row-1][col];
            const int left = buffer[row][col-1];
            const int diagonal = buffer[row-1][col-1];
